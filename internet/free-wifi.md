# Free WiFi in Istanbul

Istanbul Metropolitan Municipality (İBB) runs a free public WiFi network ("İBB WiFi") in many high-traffic public spots across the city.

## Where you'll typically find it

- Major squares and parks (e.g. around Taksim, Sultanahmet)
- Some metro/metrobus stations and public buses
- Ferries operated by Şehir Hatları
- Some municipal buildings and public libraries

Coverage is denser in central, touristy, and high-density districts, and thinner further out — don't count on it as your only source of connectivity outside the city center.

## How to connect

1. Turn on WiFi and select the İBB WiFi (or similarly named municipal) network from the list.
2. You'll be redirected to a login/captive portal page.
3. Sign in with your phone number (SMS verification) — this is the same account system used by the [İstanbul Senin app](../apps/istanbul-senin-app.md), so if you've already registered there, you may be able to log in directly instead of re-verifying by SMS.
4. Once verified, you get a free timed session; you typically need to reconnect/re-authenticate after the session expires or if you leave the hotspot's range.

## Practical notes

- A working Turkish phone number is effectively required for the SMS verification step — see [SIM Cards, eSIM & Mobile Data](sim-cards-and-mobile-data.md) for how to get one
- Speed and reliability vary a lot by location and time of day; it's fine for messaging/maps but don't rely on it for anything bandwidth-heavy
- Session limits and login flow are set by İBB and have changed over time (e.g. session length, whether the app account and WiFi login are unified) — treat the exact steps above as a general guide, not a guarantee, and expect to adapt on the spot

## Related

- [İstanbul Senin App](../apps/istanbul-senin-app.md) — the app tied to WiFi login and other municipal services
- [SIM Cards, eSIM & Mobile Data](sim-cards-and-mobile-data.md) — getting a Turkish number for SMS verification
